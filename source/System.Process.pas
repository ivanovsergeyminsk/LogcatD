...
function RunCommand(const exename: string; const commands: TArray<string>; out outputstring: ansistring; Options : TProcessOptions = []; WindowOptions: TShowWindowOptions = swoNone):boolean; overload;// L505
...
function RunProcess(p:TProcess;out outputstring: ansistring; out stderrstring: ansistring; out exitstatus:integer): integer;

implementation

{$IFDEF MACOS} //L505
  {$i system_process_macos.inc}
{$ENDIF}

{$IFDEF MSWINDOWS}  //L505
  {$i system_process_win.inc}
{$ENDIF}
...
function RunProcess(p:TProcess;out outputstring: ansistring; out stderrstring: ansistring; out exitstatus:integer): integer;
begin
  result := internalRuncommand(p, outputstring, stderrstring, exitstatus);
end;
...
function RunCommand(const exename:string; const commands: TArray<string>; out outputstring: ansistring; Options : TProcessOptions = []; WindowOptions: TShowWindowOptions = swoNone):boolean;
Var
    p : TProcess;
    i,
    exitstatus : integer;
